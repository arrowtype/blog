...
- Glyphs *also* assigns layers per-glyph, and these are date and time-stamped by default. This can be an easy way to quickly create a version history of glyphs, as you iterate on an idea. This would also be possible in UFOs, but RoboFont instead handles layers in a global sense, across a whole UFO, making them less useful for version history. (But there are better ways to handle versions and alternate glyph ideas. For versioning, save duplicate, date-stamped font files as you go, and/or use Git. For exploring which shape of a glyph to use, make duplicate glyphs, and test them in spacing strings, proofs, and marketing graphics.)